...
def get_defense_waiting_positions(nb_positions: int, radius: float = BASE_RADIUS * 1.2):
...
def find_targets(monsters: list[Entity]):
    ...
    monsters_with_threat_level = [
        (monster, get_threat_level(monster))
...
def move_to_waiting_position(hero_index: int):
    """
    hero_index should be between 0 and nb_heroes - 1
    """
    waiting_positions = get_defense_waiting_positions(heroes_per_player)
...
def generate_commands(heroes: list[Entity], monsters: list[Entity], my_mana: int) -> list[str]:
    monsters_to_attack = find_targets(monsters)
    hero_to_monster = assign_heroes_to_monsters(heroes, monsters_to_attack)

    monsters_by_id = {m.id: m for m in monsters}
    return get_commands(hero_to_monster, heroes, monsters_by_id, my_mana)
...
while True:
    ...
    commands = generate_commands(my_heroes, monsters, my_mana)
